...
params = {
    ...
    'from': yesterday,
    'to': yesterday,
    'sfx': '&timezone=Europe%2fWarsaw&additional_fields=deeplink_url,oaid,gp_referrer,gp_click_time,gp_install_begin,amazon_aid,keyword_match_type'
}
...
if res.status_code != 200:
    ...
    f = open('Uninstall_report-{}-{}-{}-to-{}-Android.csv'.format(app_id, report_type, params['from'], params['to']), 'w', newline='',
             encoding="utf-8")
    f.write(res.text)
    f.close()
...
csv_file = ('Uninstall_report-{}-{}-{}-to-{}-Android.csv'.format(app_id, report_type, params['from'], params['to']))
print(current_time, ': File named :"' + csv_file + '" successfully captured.')

with open(csv_file, 'r', encoding='utf-8') as f:
    with open('Uninstall_report_converted-Android.csv', 'w', encoding='utf-8') as f1:
        next(f)  # skip header line
        for line in f:
            f1.write(line)

print(current_time, ': Initial report processed and saved as : Uninstall_report_converted-Android.csv')

andreport = 'Uninstall_report_converted-Android.csv'
