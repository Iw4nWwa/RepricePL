...
if res.status_code != 200:
    ...
    f = open('Installs_report-{}-{}-{}-to-{}-AndroidRet.csv'.format(app_id, report_type, params['from'], params['to']), 'w', newline='',
             encoding="utf-8")
    f.write(res.text)
    f.close()
...
csv_file = ('Installs_report-{}-{}-{}-to-{}-AndroidRet.csv'.format(app_id, report_type, params['from'], params['to']))
print(current_time, ': File named :"' + csv_file + '" successfully captured.')

with open(csv_file, 'r', encoding='utf-8') as f:
    with open('Installs_report_converted-AndroidRet.csv', 'w', encoding='utf-8') as f1:
        next(f)  # skip header line
        for line in f:
            f1.write(line)

print(current_time, ': Initial report processed and saved as : Installs_report_converted-AndroidRet.csv')

andreport = 'Installs_report_converted-AndroidRet.csv'
